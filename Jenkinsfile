            ...
            container("buildbar") {
                stage('build bar') {
                    sh label: '', script: '''#!/bin/bash
                        ...
                        cd $PROJECT_DIR
                        mqsicreatebar -data . -b $BAR_NAME_${currentBuild.number}.bar -a $APP_NAME -cleanBuild -trace -configuration . 
                        ls -lha
                        '''
                }
            }
            ...
            container("oc-deploy") {
                stage('upload bar') {
                    sh label: '', script: '''#!/bin/bash
                        ...
                        cd $PROJECT_DIR
                        ls -lha
                        echo "Calling upload-barfile-to-artifactory.sh ${ARTIFACTORY_HOST} ${ARTIFACTORY_REPO} ${ARTIFACTORY_BASE_PATH} ${BAR_NAME}_${currentBuild.number}.bar ${ARTIFACTORY_USER} ${ARTIFACTORY_PASSWORD}"
                        ./upload-barfile-to-artifactory.sh ${ARTIFACTORY_HOST} ${ARTIFACTORY_REPO} ${ARTIFACTORY_BASE_PATH} ${BAR_NAME}_${currentBuild.number}.bar ${ARTIFACTORY_USER} ${ARTIFACTORY_PASSWORD}
                        '''
                }
            }
        }
            ...
            container("oc-deploy") {
                stage('deploy server') {
                    sh label: '', script: '''#!/bin/bash
                        set -e
                        cd $PROJECT_DIR
                        BAR_FILE=$BAR_NAME_${currentBuild.number}.bar
